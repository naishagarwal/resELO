...
async function get_and_set_pdf_links(auth, navigate, clubName, setPdf_1_link, setPdf_2_link) {
...
function winClick(auth, navigate, win, lose, clubName, setPdf_1_link, setPdf_2_link) {
    const user = auth.currentUser;
    if (user) { 
        auth.currentUser.getIdToken().then((idToken) => {
...
export default function Page() {
    ...
    useEffect(() => {
        get_and_set_pdf_links(auth, navigate, clubName, setPdf_1_link, setPdf_2_link)
    }, []);
    ...
    return (
        <div>
            ...
            <div className={compare_styles.contentContainer}>
                <Resume resume={pdf_1_link} onButtonClick={() => winClick(auth, navigate, pdf_1_link, pdf_2_link, clubName, setPdf_1_link, setPdf_2_link)}/>
                <Resume resume={pdf_2_link} onButtonClick={() => winClick(auth, navigate, pdf_2_link, pdf_1_link, clubName, setPdf_1_link, setPdf_2_link)}/>
            </div>
        </div>
    )
}
